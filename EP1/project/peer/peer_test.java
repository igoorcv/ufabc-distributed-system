		...
		try {
			...
			while (true) {
				...
				if (scanner.hasNextInt()) {
					...
					case 3:
						...
						if (downloadPeers.isEmpty()) {
							...
							try (Socket socket = new Socket(peerIP, peerPort)) {
								...
								String mensagem = "Envie-me o arquivo " + downloadFileName + " para download?";
								byte[] mensagemBytes = mensagem.getBytes();
																
								// Envia a requisição de download ao peer. Variavel request.
					...
					System.out.println("Opção inválida. Tente novamente.");
					scanner.next(); // Limpa o valor inválido do buffer do Scanner
				}
